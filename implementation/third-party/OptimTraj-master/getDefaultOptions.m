...
for i=1:length(opt)
    ...
    switch opt(i).defaultAccuracy
        case 'low'
            OPT.nlpOpt = optimoptions('fmincon',...
                'Display',NLP_display,...
                'StepTolerance',1e-6,...
                'MaxIterations',200,...
                'MaxFunctionEvaluations',1e4*(nState+nControl));
        case 'medium'
            OPT.nlpOpt = optimoptions('fmincon',...
                'Display',NLP_display,...
                'StepTolerance',1e-10,...
                'MaxIterations',400,...
                'MaxFunctionEvaluations',5e4*(nState+nControl));
        case 'high'
            OPT.nlpOpt = optimoptions('fmincon',...
                'Display',NLP_display,...
                'StepTolerance',1e-10,...
                'MaxIterations',800,...
                'MaxFunctionEvaluations',1e5*(nState+nControl));
        otherwise
            error('Invalid value for options.defaultAccuracy')
    end
    if isfield(opt(i),'nlpOpt')
        if isa(opt(i).nlpOpt,'optim.options.Fmincon') && ~isempty(opt(i).nlpOpt)
            names = properties(opt(i).nlpOpt);
